...
class Scheduler:
    ...
    def __init__(self, schedule_time: str = "18:00", interval_hours: int = 48):
        """
        初始化调度器

        Args:
            schedule_time: 每日执行时间，格式 "HH:MM"
            interval_hours: 执行间隔（小时），默认 48 小时
        """
        try:
            import schedule
    ...
    def set_task(self, task: Callable, run_immediately: bool = True):
        """
        设置定时任务
        ...
        # 根据间隔设置调度
        interval_hours = max(1, self.interval_hours)
        if interval_hours == 24:
            self.schedule.every().day.at(self.schedule_time).do(self._safe_run_task)
        elif interval_hours == 48:
            self.schedule.every(2).days.at(self.schedule_time).do(self._safe_run_task)
        else:
            days = max(1, round(interval_hours / 24))
            self.schedule.every(days).days.at(self.schedule_time).do(self._safe_run_task)

        logger.info(
            "已设置定时任务，间隔 %d 小时（%d 天），执行时间: %s",
            interval_hours,
            max(1, round(interval_hours / 24)),
            self.schedule_time,
        )
    ...
    def set_daily_task(self, task: Callable, run_immediately: bool = True):
        """已设置定时任务（向后兼容别名）"""
        self.set_task(task, run_immediately=run_immediately)
...
def run_with_schedule(
    ...
    Args:
        ...
        interval_hours: 执行间隔（小时），默认 48
    """
    scheduler = Scheduler(schedule_time=schedule_time, interval_hours=interval_hours)
    scheduler.set_task(task, run_immediately=run_immediately)
    scheduler.run()
...
if __name__ == "__main__":
    ...
    run_with_schedule(test_task, schedule_time="23:59", run_immediately=True, interval_hours=48)
